...
RuntimeLodLayer::RuntimeLodLayer()
    ...
    m_ShaderLibrary.Load("assets/shaders/BlinnPhong.glsl");
    m_ShaderLibrary.Load("assets/shaders/Tessellation.comp");
}
...
void RuntimeLodLayer::OnEvent(iGe::Event& event) {
    ...
    dispatcher.Dispatch<iGe::MouseMoveEvent>(
            std::bind(&RuntimeLodLayer::OnMouseMoveEvent, this, std::placeholders::_1));
}
...
bool RuntimeLodLayer::OnMouseButtonPresseddEvent(iGe::MouseButtonPressedEvent& event) {
    m_MouseButtonPressedKey = event.GetMouseButton();
...
bool RuntimeLodLayer::OnMouseButtonReleasedEvent(iGe::MouseButtonReleasedEvent& event) {
    m_MouseButtonPressedKey = iGeKey::None;
    m_LastMousePosition = glm::vec2{0.0f};
    
    return false;
}

bool RuntimeLodLayer::OnMouseMoveEvent(iGe::MouseMoveEvent& event) {
    if (m_MouseButtonPressedKey != iGeKey::MouseLeft) { return false; }
    ...
    glm::vec3 newPoint3D = glm::vec3{0.0f};
    {
        ...
        double newX = (2.0 * newMousePosition.x - width) / width;
        double newY = -(2.0 * newMousePosition.y - height) / height;
